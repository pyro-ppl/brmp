...
# samples.raw_samples :: bs
# samples.get_param :: (bs, string) -> ps
...
# TODO: Now that `get_param` and `location` close over
# `samples.raw_samples` (and therefore `samples.raw_samples` doesn't
# need passing in to them) it's less clear exactly what the purpose of
# `posterior.samples` is. Previously it had to be sufficient to
# implement those functions, but that's not longer the case. It seems
